...
# Extra imports for DOCX and PDF
...
def load_data(file_or_path) -> pd.DataFrame:
    ...
    Returns pandas DataFrame or text extracted from DOCX/PDF.
    """
    if isinstance(file_or_path, (str, Path)):
        ...
        if s == ".json":
            try:
                ...
                return pd.read_json(p, encoding="latin1")
        # DOCX/PDF support if you added it
        # ...
    ...
    for c in df.columns:
        if np.issubdtype(df[c].dtype, np.datetime64):
            ...
            try:
                ...
                parsed = pd.to_datetime(sample, format="%m-%d-%Y %H:%M", errors="coerce")
...
def suggest_prompts(df: pd.DataFrame, max_suggestions: int = 8):
    """
    Return a list of helpful, ready-to-run prompts for the dataset.
...
def prompt_to_code(prompt: str, df: pd.DataFrame):
    ...
    # Unknown -> let UI decide to use LLM
    return None

# ----------------- Execution engine -----------------
def run_code(df: pd.DataFrame, code: str):
    """
    Execute code string in a restricted local namespace. Returns a dict:
    - {"type":"text","output":...}
    ...
    try:
        ...
        # Otherwise return printed output
...
def ask_llm(prompt: str, model: str = "llama3.1", timeout: int = 60) -> str:
    """
    Send prompt to local Ollama via CLI. Returns only the Python code inside ```python blocks.
    If Ollama is missing or fails, returns an error string.
    """
    try:
        proc = subprocess.run(
            ["ollama", "run", model],
        ...
        match = re.search(r"```python(.*?)```", out, re.DOTALL)
        if match:
            code = match.group(1).strip()
            return code
        else:
            return f"[LLM-output] No python block found:\n{out}"

    except FileNotFoundError:
        return "[LLM-missing] Ollama not found. Install from https://ollama.com and add to PATH."
    except Exception as e:
